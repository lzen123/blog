...
**这段十几行的 `vue.config.js` 配置，决定了你的项目打包后是"一个 3MB 的巨石"还是"几个能被浏览器长期缓存的小块"。** 它做了三件事：把运行时（runtime）内联进 HTML、把代码按"第三方库 / UI 组件库 / 公共组件"三层切分、再把 webpack 运行时单独抽出。这篇文章逐行拆开它，讲清楚每个配置项到底在优化什么、为什么这么切。
